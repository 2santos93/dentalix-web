...
/**
 * `DashboardDto` (and its nested `DashboardSalesDto`) are fully
 ...
 *
 * The one exception is `sales.byCurrency`: on the backend it's a plain
 ...
 */
export type Dashboard = Omit<components['schemas']['DashboardDto'], 'sales'> & {
  sales: Omit<components['schemas']['DashboardSalesDto'], 'byCurrency'> & {
 ...
 * (sales totals, low-stock items, upcoming appointments, patient count) for
 * the requested period. The backend 403s any other role; callers should
 * surface `ApiError.message` for that case rather than a generic error (see
 * `DashboardView`).
 */
